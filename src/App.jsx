<artifacts>
<artifact identifier="app-jsx-firebase-integrated" type="application/vnd.ant.code" language="javascript" title="🔥 App.jsx con Firebase Integrato - Sostituisci il tuo file completo">
import React, { useState, useEffect } from 'react';
// 🔥 NUOVI IMPORT FIREBASE
...
className={p-6 rounded-2xl transition-all duration-300 transform ${             disabled                  ? 'bg-white/10 border border-white/20 cursor-not-allowed'                 : 'bg-white/20 backdrop-blur-lg border border-white/30 shadow-lg hover:bg-white/30 hover:-translate-y-1 cursor-pointer'         }}
>
...
const FascicoloSanitarioForm = ({ assistito, onSave, onClose, operators }) => {
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {operators.filter(op => op.level === 'Operativo' || op.role.includes('Coordinatore')).map(operatore => (
...
// --- 🔥 FASCICOLO OPERATORE CON FIREBASE STORAGE ---
...
const handleFileUpload = async (file, type) => {
    ...
    try {
        ...
        const result = await fileService.uploadOperatorFile(operatore.id, file, type);
        
        if (type === 'attestati' || type === 'formazione') {
            const currentFiles = operatore.fascicolo?.[type] || [];
            await onUpdateFascicolo(operatore.id, { 
                [type]: [...currentFiles, {
                    name: result.name,
                    url: result.url,
                    path: result.path,
        ...
        alert(`${type} caricato con successo su Firebase!`);
    } catch (error) {
                ...
                {activeTab === 'curriculum' && (
                    <div className="space-y-4">
                        ...
                        <div className="border-2 border-dashed border-white/30 rounded-lg p-8 text-center">
                            ...
                            >
                                {isUploading ? 'Caricamento su Firebase...' : 'Seleziona File'}
                            </label>
                            {operatore.fascicolo?.curriculum && (
                                <div className="mt-4">
                                    <p className="text-green-400">✓ Curriculum caricato: {operatore.fascicolo.curriculum.name}</p>
                                    {operatore.fascicolo.curriculum.url && (
                                        <a 
                                            href={operatore.fascicolo.curriculum.url} 
                                            target="_blank" 
                                            rel="noopener noreferrer"
...
export default function App() {
    ...
    // 🔥 USA FIREBASE HOOK invece dei mock
    const {
        assistiti,
        operatori,
        loading,
    ...
    // 🔥 LOADING SCREEN FIREBASE
    if (loading) {
        return (
            <div className="min-h-screen bg-gray-900 flex items-center justify-center">
                <div className="text-white text-center">
                    <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-cyan-400 mx-auto mb-4"></div>
                    <p className="text-xl font-semibold">Caricamento dati...</p>
                    <p className="text-sm text-gray-400 mt-2">
                        {connected ? '✅ Connesso a Firebase' : '🔄 Connessione in corso...'}
                    </p>
                    <div className="mt-4 text-xs">
                        <p className="text-cyan-300">🔥 Database: Firebase Firestore</p>
                        <p className="text-purple-300">📁 Storage: Firebase Storage</p>
                        <p className="text-green-300">🚀 Hosting: Vercel</p>
                    </div>
                </div>
            </div>
        );
    ...
    const handleSaveFascicolo = async (assistitoId, fascicoloData) => {
        try {
            await updateFascicolo(assistitoId, fascicoloData);
            // Aggiorna selectedAssistito se è lo stesso
    ...
    const handleUpdateDiario = async (assistitoId, nuovoIntervento) => {
        try {
            const nuovaEntry = {
                ...
                operator_name: nuovoIntervento.operatorNames,
            ...
            await addDiarioEntry(assistitoId, nuovaEntry);
            
            // Aggiorna selectedAssistito con la nuova entry
            if (selectedAssistito && selectedAssistito.id === assistitoId) {
                const updatedEntries = [...(selectedAssistito.diario_entries || []), nuovaEntry];
                setSelectedAssistito(prev => ({ ...prev, diario_entries: updatedEntries }));
            }
        } catch (error) {
            ...
            case 'operatori': 
                return (
                    <OperatoriEnhanced 
                        onNavigate={handleNavigation} 
                        operators={operatori} 
                        onUpdateOperatoreFascicolo={updateOperatoreFascicolo}
                    />
                );
        ...
        <main className="min-h-screen w-full bg-gray-900 bg-gradient-to-br from-gray-900 via-blue-900/40 to-gray-900 text-white font-sans flex items-center justify-center p-4 sm:p-6 lg:p-8">
            ...
            <div className="fixed top-4 right-4 z-50">
                <div className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    ...
                    {connected ? '🔥 Firebase Online' : '⚠️ Modalità Offline'}
                </div>
            </div>
